const TableBody = ({ data, onDeleteRecord }) => {
  return (
    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
      {data.map((datum) => (
        ...
        >
          ...
          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200 text-right print:hidden">
            <button
              className="border border-slate-300 p-2"
              onClick={() => {
                confirm("Are you sure to delete?") && onDeleteRecord(datum.id);
              }}
            >
              <svg
